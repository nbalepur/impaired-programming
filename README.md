...
```bash
git clone https://github.com/<redacted>/impaired-programming.git
...
6. `user_code.json`: The main dataset of user background ability, websites, and comprehension scores
...
You can easily spin up Jupyter Notebook with uv:

```bash
uv run jupyter notebook # launch the jupyter notebook
```

You can then walk through each Jupyter notebook to reproduce the plots in our paper, including our hypothesis tests, regressions, and qualitative analyses
...
```bash
uv run python llm_judge/label_prompts.py # label user prompts
```
...
├── analysis/                          # Reproduce paper analyses (Jupyter Notebook)
